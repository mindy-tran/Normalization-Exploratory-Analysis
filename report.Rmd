...
verse_counts <- filter_zero_var_genes(verse_counts)
...
sample_names <- colnames(verse_counts)[-1]
...
reads_cts <- get_library_size(verse_counts)
reads_cts
#Make a barplot showing the number of millions of reads for each sample
ggplot(reads_cts, aes(x=fct_inorder(sample), y=value/1000000, fill=sample)) +
  geom_bar(stat="identity") +
  ylab('# reads') + # for the y axis label
  xlab('Sample') # for the x axis label
...
verse_counts_cpm <- normalize_by_cpm(verse_counts)
head(verse_counts_cpm)
...
verse_counts_dds <- deseq_normalize(verse_counts, meta)
head(verse_counts_dds)
```
...
plot_sample_distributions(verse_counts, TRUE, 'Sample Distribution (Log10(Raw Counts))')
```
...
plot_variance_vs_mean(verse_counts, TRUE, 'Variance vs Mean (Log10(Raw Counts))')
...
plot_pca(verse_counts, meta, "Raw Count PCA")
...
plot_sample_distributions(verse_counts_cpm, TRUE, 'Sample Distribution (log10(CPM))')
plot_variance_vs_mean(verse_counts_cpm, TRUE, 'Variance vs Mean (log10(CPM))')
plot_pca(verse_counts_cpm, meta, "CPM PCA")
...
plot_sample_distributions(verse_counts_dds, TRUE, 'Sample Distribution (log10(DESeq2 norm counts)')
plot_variance_vs_mean(verse_counts_dds, TRUE, 'Variance vs Mean (DESeq2)')
plot_pca(verse_counts_dds, meta, "DESeq2 Normalized PCA")
```
